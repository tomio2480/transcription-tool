...
- whisper.cpp のバイナリとモデル（計約 3.6 GB）は `blog-private/.scratch/stt-pilot/` にあった．
...
- 既定ディレクトリは Windows で `%LOCALAPPDATA%\transcription-tool\` とした．
  それ以外は `$XDG_DATA_HOME/transcription-tool` とする．
  未設定なら `~/.local/share/transcription-tool` に置く．
...
- `.part` へ書きながらハッシュを計算し，一致時のみ `os.replace` で確定する．不一致・例外時は `.part` を消す．
  ただしプロセスの強制終了では `finally` が走らない．`.part` は残るが，再実行時に上書きされるため実害はない．
...
- `which: Callable = shutil.which` のようにデフォルト引数へ直接束縛してはならない．
  テストで `shutil.which` を monkeypatch しても効かない．定義時に評価済みの参照を持つためである．
